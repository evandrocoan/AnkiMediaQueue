...
class AnkiMediaQueue {
    ...
    _validateWhere(where, caller, media = undefined) {
        ...
        if (!where) {
            throw new Error(
                `ankimedia.${caller}: Missing the 'where=${where}' parameter!\n${fix_message} ` +
                    this._getMediaInfo(media)
            );
        }
        if (!(where == "front" || where == "back")) {
            throw new Error(
                `ankimedia.${caller}: Invalid 'where=${where}' parameter!\n${fix_message} ` +
                    this._getMediaInfo(media)
            );
        }
    }
    ...
    _validateSpeed(location, speed, media = undefined) {
        if (typeof speed != "number" || speed <= 0) {
            throw new Error(
                `ankimedia.${location}: The 'speed=${speed}/${typeof speed}' is not a valid positive number.` +
                    this._getMediaInfo(media)
            );
        }
    }
    ...
    add(htmlfield, where = undefined, speed = undefined) {
        ...
        if (!(typeof htmlfield == "string")) {
            throw new Error(
                `The ankimedia.add() 'htmlfield=${htmlfield}/${typeof htmlfield}' is not a valid string. ` +
        ...
        if (media) {
            ...
            if (playpromise) {
                playpromise.catch((error) =>
                    console.log(
                        `ankimedia: Could not play '${filename}' due to '${error}'! ` +
                            this._getMediaInfo(media)
                    )
                ).then(_ => {
                ...
                console.log(
                    `ankimedia: Could not play the media '${filename}'! ` +
                        this._getMediaInfo(media)
                );
            }
            this._startnext = (event) => {
    ...
    _getMediaElement(location: string, filename: string, selected: Map<string, number>) {
    ...
    __getMediaElement(location: string, filename: string, selected: Map<string, number>) {
    ...
    setup(parameters: any = {}) {
        ...
        if (typeof parameters != "object") {
            throw new Error(
                `ankimedia.setup: Invalid 'parameters=${parameters}/${typeof parameters}' passed to setup!`
            );
        }
        for (let [key, value] of Object.entries(parameters)) {
            if (!(key in default_parameters)) {
                throw new Error(
                    `ankimedia.setup: Invalid 'parameters=${key}-${value}/${typeof key}' passed to setup!`
                );
            }
        }
        ...
        if (!Array.isArray(medias)) {
            throw new Error(
                `ankimedia.setup: The 'medias=${medias}/${typeof medias}' is not a valid array object!`
            );
        }
        if (typeof delay != "number" || delay < 0) {
            throw new Error(
                `ankimedia.setup: The 'delay=${delay}/${typeof delay}' is not a valid positive number!`
            );
        }
        if (typeof extra != "function" && extra != undefined) {
            throw new Error(
                `ankimedia.setup: The 'extra=${extra}/${typeof extra}' is not a valid function!`
            );
        }
    ...
    _checkDataAttributes(location: string, media: HTMLMediaElement) {
        let data_file = this._getSource(media);
        let data_speed = media.getAttribute("data-speed");

        if (typeof data_file != "string") {
            throw new Error(
                `ankimedia.${location}: A media element is missing its 'src=${data_file}' attribute. ` +
                    this._getMediaInfo(media)
            );
        }
        ...
        if (timmed_data_file != data_file) {
            throw new Error(
                `ankimedia.${location}: A media element has leading or trailing whitespaces on its 'src=${data_file}' attribute. ` +
                    this._getMediaInfo(media)
            );
        }
        if (
            ...
            throw new Error(
                `ankimedia.${location}: A media element has an invalid 'data-speed=${data_speed}/${typeof data_speed}' attribute. ` +
                    this._getMediaInfo(media)
            );
        }
        ...
        if(playing_media && playing_media.paused) {
            ...
            if (playpromise) {
                playpromise.catch((error) => {
                    this._is_playing_resuming = false;
                    console.log(
                        `ankimedia.togglePause: Could not unpause the media due to '${error}'! ` +
